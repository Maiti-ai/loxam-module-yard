-- Production zone F is 1-high. Storage blocks stay 3-high.
-- Mirrors src/config/yard.ts BLOCK_MAX_STACK_LEVELS.

create or replace function public.max_stack_levels_for_block(p_code text)
returns integer
language sql
immutable
as $$
...
begin
  ...
  v_max_stack := public.max_stack_levels_for_block(v_block_code);
  ...
  select ml.module_id
    into v_extra
  from public.yard_slots s
  join public.module_locations ml on ml.slot_id = s.id
  where s.position_id = p_position_id
    and ml.module_id is distinct from p_module_id
    and case s.level
      when 'GROUND' then 0
      when 'LEVEL_1' then 1
      when 'LEVEL_2' then 2
...
revoke all on function public.max_stack_levels_for_block(text) from public;
grant execute on function public.max_stack_levels_for_block(text) to authenticated;
