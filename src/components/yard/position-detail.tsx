...
export function PositionDetailPanel({
  ...
  const capacity = positionCapacity(position, stackOptions);
  const full = isStackFull(position.levels, stackOptions) || capacity.occupied >= capacity.total;
  const inconsistent = hasInconsistentStack(position.levels, stackOptions);
  ...
  return (
    <div className="space-y-4 border-4 border-loxam-black bg-white p-4">
      ...
      {stacked ? (
        ...
        <ProductionStatus position={position} />
      )}

      {inconsistent ? (
        <p className="text-sm font-bold text-loxam-occupied">
          {stacked ? t("move.inconsistentStack") : t("move.inconsistentProductionStack")}
        </p>
      ) : null}
      ...
      <div className="border-t-4 border-loxam-black pt-3">
        ...
        <p className="mt-1 text-sm font-black uppercase">
          {t(capacity.total === 1 ? "move.capacitySingular" : "move.capacity", capacity)}
        </p>
        <p className="mt-1 text-sm font-bold text-loxam-muted">
          {t("yard.availableSlots", {count: capacity.available})}
        </p>
        {full ? (
          <p className="mt-2 text-base font-black text-loxam-occupied">
            {stacked ? t("yard.positionComplete") : t("yard.positionOccupied")}
          </p>
        ) : null}
      </div>
...
function ProductionStatus({position}: {position: YardPositionNode}) {
  const t = useTranslations();
  const occupant =
    position.levels.find((item) => item.level === "GROUND")?.occupant ??
    position.levels.find((item) => item.occupant)?.occupant ??
    null;

  return (
    <div className="space-y-2">
      <p className="text-xs font-bold tracking-[0.2em] text-loxam-muted uppercase">{t("module.status")}</p>
      <div className="flex min-h-16 items-center justify-between gap-3 border-2 border-loxam-line px-3 py-2">
        {occupant ? (
          <>
            <p className="text-lg font-black text-loxam-occupied">{t("common.occupied")}</p>
            <Link
